...
scope:
  ...
  - description: |
      ...
      Map-backed, constructor-injectable style so tests can inject a fresh instance per
      test the same way `test/login.test.ts` injects a fresh `SessionRepository`.
  ...
  - description: |
      ...
      `verifyAccessToken` from the existing auth module (same Bearer-token pattern as
      `handleGetSession` in `src/auth/authController.ts`).
...
assumptions_or_open_questions:
  ...
  - |
    ...
    exists). `evaluateCancellation` therefore takes raw `appointmentTimeMs`/`cancellationTimeMs`
    timestamps directly rather than a booking ID, and is exposed only as an importable
    service method — not a new HTTP endpoint — for a future Booking module to call. Actual
    refund/fee processing is out of scope per the parent epic ("delegated to Payments &
    Invoicing"); this story only determines the outcome.
  - |
    ...
    (`src/auth/tokenService.ts`). No new authorization mechanism is introduced, and no
...
notes: |
  ...
  already used for `userRepository`/`sessionRepository`.
